...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        mec = Orgao.objects.create(nome="Ministério da Educação", sigla="MEC", descricao="Órgão central de educação nacional", ativo=True)
        saude = Orgao.objects.create(nome="Ministério da Saúde", sigla="MS", descricao="Gestão de saúde pública", ativo=True)
        ifrn = Orgao.objects.create(nome="Instituto Federal do Rio Grande do Norte", sigla="IFRN", descricao="Ensino técnico e superior", ativo=True)
        ...
        def grant_perms(group, app_label, model_name, actions=['add', 'change', 'delete', 'view']):
            try:
                ...
                self.stdout.write(self.style.WARNING(f"Erro ao atribuir permissão {app_label}.{model_name}: {str(e)}"))

        # Configurando permissões por grupo
        for app in ['cadastro', 'editorial', 'atendimento', 'eventos', 'media_library', 'public_forms']:
        ...
        grant_perms(eventos_manager, 'eventos', 'inscricaoevento', ['change', 'delete', 'view'])
        ...
        cat_noticias = Categoria.objects.create(nome="Notícias Gerais", slug="noticias-gerais")
        cat_editais = Categoria.objects.create(nome="Editais e Licitações", slug="editais-licitacoes")
        ...
        Comentario.objects.create(noticia=n1, autor="José da Silva", comentario="Parabéns pela iniciativa! Facilita muito nossa vida.")
        Comentario.objects.create(noticia=n1, autor="Maria Santos", comentario="Gostaria de saber se o aplicativo mobile também receberá atualização.", aprovado=False)
        ...
        s1 = Servidor.objects.create(orgao=ifrn, nome="Carlos Eduardo Medeiros", cpf="111.111.111-11", email="carlos.medeiros@ifrn.edu.br", cargo="Professor Titular", data_admissao=datetime.date(2015, 3, 10))
        s2 = Servidor.objects.create(orgao=mec, nome="Ana Julia Barbosa", cpf="222.222.222-22", email="ana.barbosa@mec.gov.br", cargo="Analista Técnico Administrativo", data_admissao=datetime.date(2018, 6, 15))
        s3 = Servidor.objects.create(orgao=saude, nome="Roberto Carlos Souza", cpf="333.333.333-33", email="roberto.souza@saude.gov.br", cargo="Médico Clínico", data_admissao=datetime.date(2012, 1, 1))
        ...
        servico = Servico.objects.create(nome="Emissão de Diploma Digital", url_portal="https://www.gov.br/pt-br/servicos/emissao-diploma-digital", publico_alvo="Estudantes de graduação")
        ...
        InscricaoEvento.objects.create(evento=e1, nome_participante="Guilherme Moreira", email_participante="guilherme@gov.br", pago=True, valor_pago=50.00)
        InscricaoEvento.objects.create(evento=e1, nome_participante="Fernanda Lima", email_participante="fernanda@gmail.com", pago=False)
        InscricaoEvento.objects.create(evento=e2, nome_participante="Carlos Barbosa", email_participante="carlos@saude.gov.br", pago=True, valor_pago=0.00)
        ...
        SolicitacaoServico.objects.create(nome="João Santos Souza", email="joao@gmail.com", telefone="(61) 98765-4321", tipo="duvida", descricao="Como solicito a segunda via do meu histórico acadêmico pelo portal?", concorda_termos=True)
        SolicitacaoServico.objects.create(nome="Clara Mendes Reis", email="clara.reis@hotmail.com", telefone="(84) 99123-4567", tipo="reclamacao", descricao="A página de carregamento de anexo de diploma demorou mais de 5 minutos e depois deu timeout.", concorda_termos=True)
